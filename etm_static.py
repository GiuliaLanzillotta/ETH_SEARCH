...
ckpt = os.path.join(results_path,
                    'etm_K_{}_Htheta_{}_Optim_{}_Clip_{}_ThetaAct_{}_Lr_{}_Bsz_{}_RhoSize_{}_trainEmbeddings_{}'.format(
                        num_topics, t_hidden_size, _optimizer, clip, theta_act, lr, training_batch_size, rho_size,
                        train_embeddings))
...
def process_subset_static(doc_subset, tokenizer, model,
    ...
    for i in range(subset_size):
        t1 = time.time()
        embedded_doc = lower_hiddens[i][
            tokenised_collection["attention_mask"][i].bool()]  # removing padding using the attention mask
        ...
        for j, emb_vector in enumerate(embedded_doc):
            ...
            if word not in word2uniquevec.keys():  # only consider words without unique vector representation (i.e. new words)
                if word not in word2manyvec.keys():  # create new entry if we encounter word for the first time
                    ...
                    word2manyvec[word] = np.append(word2manyvec[word], [emb_vector.detach().numpy()],
                                                   axis=0)  # note: using numpy array as lists manipulate inplace
...
def do_processing(tokenizer, model, load_from_file, idx2word, word2uniquevec):
    if load_from_file:
        ...
        with open(os.path.join(results_path, args.vocab), "rb") as fp:
            idx2word = pickle.load(fp)
        with open(os.path.join(results_path, args.embedding), "rb") as fp:
        ...
        with open(os.path.join(results_path, args.embedding), "wb") as fp:
            pickle.dump(embedding, fp)
        with open(os.path.join(results_path, args.tokens), "wb") as fp:
